...
private:
	...
	int blockSize = 20, locationNumber = 0, positionCounter = 0, vectorSize = 4, pointsCounter = 0, points = 0, randomBlock = rand() % 7 + 1, currentRandomBlock, pointsMover = 10, rowCounter = 0, rowNumber = 0, rowPoints = 0, level = 0, rowsCleared = 0, red = rand() % 255 + 1, blue = rand() % 255 + 1, green = rand() % 255 + 1, currentFrame = 0, frameCount = 6;
	Blocks* block;
	sf::Clock clock;
	...
	std::vector<sf::Sprite> spriteVector, allSprites, flames;
